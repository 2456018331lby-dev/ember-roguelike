...
最后更新：2026-06-09（本轮优先回应前端质感问题，新增本地生成的主菜单封面资产 `web/assets/ember-menu-tableau.png`，由 `scripts/generate-art-assets.mjs` 稳定生成角色队伍、Boss 剪影、卡牌碎片和火光构图；主菜单改为全屏游戏封面式入口，桌面端标题/按钮/右侧存档 HUD 覆盖在封面上，移动端继续保证开始按钮首屏可见。追加更新：结算页已新增受击来源统计，核心会累计 Boss 弹幕、普通弹幕、近身和自伤等实际扣血来源，结果页用条形占比展示“瞄准连射”等主要来源，并给出横向走位/冲刺窗口建议；本轮继续新增“卡组路线对比”，结果页会把实际路线、四类压力达标/缺口、最大最终短板和下一把修正建议放在同一块读板里；奖励页也完成首轮卡牌动效和质感增强，新增卡面光泽、契合度条、风险刻度、机会锚点和低高度桌面紧凑布局。最新几轮完成高波 Boss 阶段特效、角色选择战术档案和奖励高稀有度反馈首轮强化：高波样本有边界压力带、阶段阈值环和 Boss 阶段冠环；角色选择新增大头像、当前档位、起手铭牌、属性条、紧凑名册和 sticky 开始按钮；奖励页史诗/传说卡新增阶级 rail、罗马数字 sigil 和传说推荐卡动效。视觉 smoke 和视觉回归已纳入桌面/移动端结果页、奖励页高稀有度反馈、高波 Boss 和角色选择变化。已完成 `npm test`、`npm run test:web-smoke`、`npm run test:boss-checkpoints`、`npm run test:visual-smoke`、`npm run test:visual-regression -- --update`、`npm run test:visual-regression`。下一步继续把敌人多帧动画、协同触发反馈和高风险高回报卡牌诱惑感推进到更接近成品。）
...
- 奖励高稀有度卡已完成首轮专属反馈：史诗/传说卡会显示阶级 rail 和罗马数字 sigil，传说推荐卡有轻量呼吸动效，降低动效系统下会自动关闭
- 奖励 / 锻造 / 商店 / 营火点击后的 UI 状态同步已集中到 `syncOverlayForRunState`
- 角色选择页和局内玩家模型已换成 AI 生成 spritesheet，明显弱化了原先的占位感
- 角色选择页现在会直接展示职业职责、武器、战斗备注和起手流派；本轮新增选中角色战术档案，展示大头像、当前档位、起手铭牌和四项属性条，桌面/移动端首屏都能看到 sticky 开始按钮
...
- `scripts/visual-smoke.mjs` 已可用本机 Chrome/Edge 做桌面/移动端视觉 smoke 并输出截图，覆盖菜单、角色选择、局内 canvas、奖励选择页读板、Boss 前营火、第 5 波 Boss 进入、桌面/移动端活跃 Boss 战读招画面、固定的第 20 波高压桌面样本、桌面/移动端结算复盘、路线对比、受击来源区域，以及桌面/移动端时间线区域；菜单样本会断言本地生成封面图已加载且 hero 面板全幅铺满；角色选择样本会断言 `char-dossier`、4 个属性条、3 个起手铭牌、当前档位和开始按钮视口可见；高波样本还会断言 debug snapshot 中存在当前 `bossPatternLabel`、`bossCharge`、`bossPhaseLabel`、`bossPhaseIndex`、`bossPhaseTotal` 和 `bossPhaseIntensity`，奖励/营火样本会断言存在“下一波作战计划”、压力优先级和 chips，每张奖励卡有 `card-readout`、机会标签、风险等级、稀有度阶级、契合度条、风险刻度、机会锚点、rail/sigil 且没有内部裁切，并要求至少一张高稀有度卡和一张传说卡；结果页样本会断言崩盘诱因包含具体高伤害命中，路线对比样本会断言实际路线、最终缺口、达标项和缺口项存在，受击来源样本会断言 Boss 连射、累计伤害和走位建议，结果时间线样本会断言存在路线影响、具体高伤害命中来源、战斗事件和崩盘节点
...
- `npm run test:visual-smoke`：桌面/移动端菜单、角色选择、局内 canvas、奖励选择页、Boss 前营火、第 5 波 Boss 进入、桌面/移动端活跃 Boss 战读招画面、固定第 20 波高压桌面样本、桌面/移动端结算复盘、路线对比、受击来源区域，以及桌面/移动端时间线区域渲染通过；菜单样本已断言 `ember-menu-tableau.png` 加载与全幅铺满，角色选择样本已断言 `char-dossier`、属性条、起手铭牌、当前档位和开始按钮视口可见，高波样本已断言 `bossPatternLabel` / `bossCharge` / `bossPhaseLabel` / `bossPhaseIntensity`；奖励页会断言每张卡有稀有度阶级、rail/sigil、契合度条、风险刻度、机会锚点且无内部裁切，并要求至少一张高稀有度卡和传说卡；结果页会断言摘要标题/说明未被裁切、底部按钮在视口内、崩盘诱因显示具体高伤害命中；路线对比会断言实际路线、最终缺口、四个压力项、达标项和缺口项；受击来源会断言累计伤害、至少两条来源、Boss 连射和横向走位/冲刺建议；时间线会断言存在路线影响、具体高伤害命中来源、战斗事件和崩盘节点；截图已输出到 `output/visual-smoke/`
...
- 奖励页已有首选/豪赌层级、结构化读板、契合度条、风险刻度、首轮卡面动效和高稀有度专属反馈；下一步可以继续强化协同触发和高风险高回报选择的专属动画反馈
- 主菜单、高波 Boss 阶段压迫、角色选择战术档案和奖励高稀有度反馈已经完成首轮强化，但整体视觉完成度还没到真正成品级；卡牌诱惑感和敌人多帧动作仍是下一批主要前端缺口
