...
- 主菜单已接入本地生成的 1280x720 封面图：角色队伍、Boss 剪影、卡牌碎片和火光元素由资源脚本稳定生成，桌面端改成全屏游戏封面式入口，移动端保持首屏按钮可见
...
- 结算页已升级为战报复盘，会记录最后几次关键选择、路线影响、高伤害命中来源、受击来源占比、走位建议、崩盘诱因、复盘时间线、数值缺口和“下一把优先级”，帮助判断是输出、清场、续航、安全网、具体受击还是路线选择导致阵亡
...
- `npm run test:web-smoke` 会验证 spritesheet 和菜单封面图尺寸/内容、service worker 预缓存清单、本地服务 MIME、UI 状态路由，以及核心逻辑发出的粒子类型是否都被前端绘制，不需要新增浏览器测试依赖。
- `npm run test:visual-smoke` 会用本机 Chrome/Edge 做桌面和移动端视觉 smoke，覆盖菜单、角色选择、局内 canvas、奖励选择页读板、Boss 前营火、Boss 波进入、桌面/移动端活跃 Boss 战读招画面、固定的第 20 波高波 Boss 桌面场景、桌面/移动端结算复盘、受击来源区域，以及桌面/移动端时间线区域；菜单会检查本地生成封面图已加载且首屏全幅铺满，奖励/营火页会检查“下一波作战计划”、每张卡的读板、机会标签、风险等级且无内部裁切，结果页会检查摘要未被裁切、底部按钮可见、崩盘诱因显示具体高伤害命中、受击来源含 Boss 连射占比和走位建议、时间线存在路线影响、具体高伤害命中来源、战斗事件和崩盘节点，截图输出到 `output/visual-smoke/`，不需要新增 Playwright/Puppeteer 依赖。
- `npm run test:visual-regression` 会先跑视觉 smoke，再读取 PNG 像素并和 `tests/visual-regression-baseline.json` 对比；角色选择等稳定界面使用严格 RGBA 哈希，动画界面使用像素亮度/色彩比例容差，当前基线已包含奖励页读板、结算页、受击来源区域和时间线区域的桌面/移动端截图。
...
- `npm run verify:android:smoke` 会在 `debug` 验证基础上继续点击 `开始远征 -> 开始战斗`，确认 APK 能进入局内战斗，并保存启动、角色选择、局内三张截图；角色选择和局内截图必须超过最低体积阈值，避免把 Capacitor 启动画面或黑屏误判成成功。
- `npm run assets:generate` 默认会保留现有角色 spritesheet，并重生成参考图风格竞技场、敌人 / Boss 图集和主菜单封面图；如确需恢复占位角色，显式传 `-- --force-placeholder-heroes`。替换同名竞技场 PNG 时，canvas 会按图片比例居中裁切铺满，不会拉伸变形。

## Android APK 构建

当前仓库已经验证过可在本机生成 debug APK。本轮当前 APK 已完成构建、APK Web payload 校验，并通过在线 Android 模拟器 `emulator-5554` 的安装、冷启动、焦点窗口、点击进入角色选择、点击进入局内战斗、截图和 logcat 致命错误扫描。

最近一次构建 APK SHA256：`F027BEF9B71BA6CF6A5EE2940B801EA037EC068B83B3442573DCB7A7F2DBDF59`

最近一次安装 / 冷启动 / 点击流验证 APK SHA256：`F027BEF9B71BA6CF6A5EE2940B801EA037EC068B83B3442573DCB7A7F2DBDF59`
...
- 真机或模拟器启动验证依赖 Android SDK `platform-tools` 和 Android Emulator。脚本会从 PATH、`android/local.properties`、`ANDROID_HOME`、`ANDROID_SDK_ROOT` 和常见本机 SDK 目录自动查找 `adb.exe`；如果没有在线设备但只有一个 AVD，npm 验证脚本还需要可发现 `emulator.exe` 才能自动启动它。本轮使用临时 PATH `C:\Users\24560\Desktop\study\Englishdemo\.android-sdk\platform-tools` 解析到 `adb.exe`，并在已在线的 `emulator-5554` 上完成点击流 smoke。
