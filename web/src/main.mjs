import { createRun, updateRun, applyCardChoice, rerollRewardChoices, getPlayerStats, dash } from './game_core.mjs';
...
function showReward() {
  ...
  if (rerollBtn) {
    ...
    rerollBtn.textContent = (run.rewardRerolls ?? 0) > 0 ? '重随奖励' : '重随耗尽';
    rerollBtn.onclick = () => {
  ...
  for (const card of run.rewardChoices) {
    ...
    el.innerHTML = `
      <div>
        ...
        <div class="fit-line">建议：${card.fitHint || '提供通用数值'}${typeof card.fitScore === 'number' ? ` · 契合度 ${card.fitScore.toFixed(1)}` : ''}</div>
      </div>
...
function showResult(victory) {
  ...
  const deathTip = presentation.deathBuildTip ? `<div class="final-stat hint"><span>💡 建议</span><span>${presentation.deathBuildTip}</span></div>` : '';
...
function updateHud() {
  ...
  if (buildFocusText) buildFocusText.textContent = `${presentation.buildSummary} · ${presentation.weaknessSummary}`;
  if (buildRiskText) buildRiskText.textContent = presentation.waveRiskLine || '保持走位，别让怪物叠到脸上。';
  if (waveSummaryText) waveSummaryText.textContent = presentation.waveSummary || '';
...
function drawArena() {
  ...
  ctx.globalAlpha = 1;
  ctx.strokeStyle = waveKind === 'boss' ? '#7f1d1d' : '#1e293b';
  ctx.lineWidth = 4;
...
function drawPlayer() {
  ...
  drawSymbol(`icon-player-${ch.id}`, run.player.x, run.player.y, 46 * pulse, 46 * pulse, run.player.invuln > 0 ? 0.7 : 1);
...
function drawEnemies() {
  for (const e of run.enemies) {
    ...
    if (e.isBoss) {
      ctx.save();
      const bossGlow = ctx.createRadialGradient(e.x, e.y, 12, e.x, e.y, e.radius * 2.8);
      bossGlow.addColorStop(0, 'rgba(239,68,68,0.16)');
    ...
    drawSymbol(symbol, e.x, e.y, e.radius * (e.isBoss ? 2.5 : 2.2), e.radius * (e.isBoss ? 2.5 : 2.2), e.phased ? 0.35 : 1);
    ...
    if (e.isElite) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(e.x, e.y, e.radius + 8 + Math.sin(gameTime * 5) * 1.5, 0, Math.PI * 2);
      ctx.stroke();
      ctx.restore();
    }
    ...
    if (e.hp < e.maxHp || e.isBoss) {
      const bw = e.radius * (e.isBoss ? 2.9 : 2.2), bh = e.isBoss ? 8 : 4, by = e.y - e.radius - (e.isBoss ? 18 : 14);
