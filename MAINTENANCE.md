...
最后更新：2026-06-08（本轮继续推进“关键选择因果 -> 复盘路线影响”的解释闭环：`game_core.mjs` 会把奖励与营火选择的 `fitHint / fitScore / decisionLabel` 写入 `decisionLog`；`presentation.mjs` 在合并 `decisionLog` 与 `combatLog` 的 `resultTimeline` 时，额外为奖励、营火、锻造和商店节点生成 `impactLabel / impactDetail / impactTone`，把当时选择和最终最大缺口（`singleTarget / aoe / sustain / safety`）对照起来，例如“清场补强，但最终最大缺口落在安全网”。`main.mjs` 和 `styles.css` 新增紧凑的“路线影响”时间线条，不改变战斗数值或选择评分；`scripts/visual-smoke.mjs` 现在断言结果时间线同时含有路线影响、战斗事件和崩盘节点。Playwright 本地浏览器通过 `?debug=1` 触发 `showResult(false)`，确认 3 条路线影响、2 条战斗事件且无水平溢出；visual smoke 与 visual regression 基线已同步。PWA cache 已提升到 `ember-v20`，并已同步 `web -> docs -> android`。当前 checkpoint 仍为 `baseline avgWave 19.12`、`smart avgWave 23.42`，smart 第 5 波 `60/60` 通过，第 20 波到达 `53/60`，第 25 波到达 `48/60`，通关 `45/60`，无 max-tick 卡局。本轮 Android debug APK 构建和 payload 校验通过，当前 APK SHA256 为 `5A6A352809BA07C893998388D0D44A7922CA8544FE1FF9CFA11BDA22A6B75B93`；临时 PATH 可解析 `adb.exe`，但本机仍找不到 `emulator.exe` 且 `adb devices` 无在线设备，当前 APK 的模拟器点击流 smoke 仍需恢复 Android Emulator 或接入真机后重跑，最近一次通过的模拟器点击流仍是前序 APK `1BCDFAF64F2EDEC1C1C1731E353629C4C6F873024C6CA3FC348B8B5204386D51`。前序记录中的奖励读板、下一把优先级、中后期奖励短板修复、Boss 前容错、脆皮 `fragilityDebt`、烟幕残影保命、Boss 读招面板、逐资源 service worker 预缓存和视觉回归仍继续成立。）
...
- `web/src/main.mjs`：新增 `renderResultTimeline()`，只消费展示层给出的 timeline 节点；本轮会渲染“路线影响”副行，UI 不重新推导构筑短板。
- `web/src/main.mjs`：奖励卡现在渲染 `card-readout`，展示协同/修复/风险机会说明；不再保留只在奖励卡里使用的 `cardStatsLine()`。
- `web/styles.css`：结算页改为标题、可滚动复盘内容和底部固定操作区的布局，移动端关键数值两列显示，摘要卡显式防裁切；本轮新增时间线卡片样式、战斗事件色调、“路线影响”色调、奖励卡读板样式、协同/修复/安全网/高风险状态边框，并恢复移动端奖励卡稳定高度防裁切。
- `scripts/visual-smoke.mjs` / `scripts/visual-regression.mjs`：新增桌面/移动端结算页、时间线区域和奖励卡读板覆盖，断言摘要标题/说明未被裁切、底部按钮在视口内、时间线包含路线影响、崩盘节点和战斗事件、奖励卡有机会标签/风险等级且卡面内容不被内部裁切，并把结果页、时间线和奖励卡变化纳入视觉回归基线。
- `web/sw.js`：PWA cache 提升到 `ember-v20`；`docs/` 镜像已由 `node scripts/sync-web.mjs` 同步。
...
- 继续强化 `构筑缺口 -> 奖励推荐 -> 战前准备 -> 波次风险提示 -> 路线影响/战斗转折复盘 -> 下一把优先级` 这一条闭环。
...
- 结算页已新增复盘时间线：奖励、锻造、商店、营火等关键选择会按波次展示，并用“路线影响”副行说明该选择和最终短板之间的关系；低血线/Boss 终局弹幕等战斗事件会插入时间线，末尾追加崩盘节点，帮助玩家把路线选择、波中转折和死亡原因连起来
...
- `scripts/visual-smoke.mjs` 已建立无 npm 依赖视觉 smoke，可用本机 Chrome/Edge 验证桌面/移动端菜单、角色选择、战斗 canvas 非空、奖励选择页推荐卡渲染和读板、Boss 前奖励点击后进入营火、营火选择后进入第 5 波 Boss、桌面/移动端结算复盘和时间线区域，并额外保存桌面和移动端活跃首领战 `boss-fight-*.png`，以及固定的高波桌面场景 `boss-fight-highwave-desktop.png`；高波样本会断言 debug snapshot 中存在当前 `bossPatternLabel` 和 `bossCharge`，奖励样本会断言每张卡有 `card-readout`、机会标签、风险等级且没有内部裁切，结果页样本会断言摘要未裁切、底部按钮可见，时间线样本会断言存在路线影响、战斗事件和崩盘节点
...
- `web/sw.js` 已预缓存 `main.mjs` 的静态模块依赖和核心美术资源；当前 `CACHE = ember-v20`；替换同名 PNG 或修改核心脚本后必须继续提升 `CACHE` 版本，避免 PWA/Android WebView 继续命中旧缓存；预缓存必须逐资源容错，不能使用 `cache.addAll()`，否则 Android WebView 可能因单个 Cache 内部错误产生启动期 fatal log；fetch handler 只拦截同源 GET，离线 cache miss 会返回明确 Response，避免 WebView console 噪声
...
- `npm run test:visual-smoke`：
  ...
  - 时间线样本会断言至少 2 条“路线影响”副行，同时保留战斗事件和崩盘节点
...
- 浏览器可视验证：
  ...
  - Playwright MCP 打开 `http://127.0.0.1:5174/?debug=1` 后用本地 debug hook 展示奖励页，snapshot 确认三张奖励卡都有完整 `card-readout`、机会说明和风险脚注；本轮同一路径触发 `showResult(false)`，确认结果页有 3 条路线影响、2 条战斗事件且无水平溢出；临时本地服务已关闭
- 难度抽样（30 seeds，自动选牌）：
...
- Android 构建：
  ...
  - 当前构建 APK SHA256：`5A6A352809BA07C893998388D0D44A7922CA8544FE1FF9CFA11BDA22A6B75B93`
...
- Android 模拟器安装 / 启动 / 点击流：
  ...
  - 没有在线设备时，脚本需要可发现 `emulator.exe` 才能自动启动唯一 AVD `NightRunner35`
...
4. 恢复本机 Android Emulator 组件（需要 `emulator.exe`）或接入在线设备后，复测当前 APK 安装启动和点击进入局内战斗；实体真机复测仍未完成
...
- 目前最真实的剩余平台风险，不是“构建不了”，而是“重大改动后要有可用设备或 `emulator.exe` 来重跑安装 / 启动 / 点击流验证”
